...
def generate_charts():
    # Gráfico de Dona (Green Score vs margen restante)
    fig, ax = plt.subplots(
...
def create_pdf_report(filename=None, export_format="both", lang=None):
    ...
    if export_format == "json":
        json_filename = export_json_report(filename, "inicio", SHARED, REPORT)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Exporta el informe de Inicio (Impacto de Carbono y Green Score)")
    parser.add_argument(
